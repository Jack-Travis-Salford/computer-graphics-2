...
raaFacarde::raaFacarde(osg::Node* pPart, osg::Vec3 vTrans, float fRotX, float fRotY, float fScale) : m_pTranslation(0), m_pRotation(0), m_pScale(0), m_pRoot(pPart)
{
	init();
	...
	m.makeTranslate(vTrans);
	if (m_pTranslation) m_pTranslation->setMatrix(m);
	m.makeRotate(osg::DegreesToRadians(fRotX), osg::Vec3(1.0f, 0.0f, 0.0f));
	if (m_pRotation) m_pRotation->setMatrix(m);
	m.makeScale(fScale, fScale, fScale);
	if (m_pScale) m_pScale->setMatrix(m);
}
